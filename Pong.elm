...
update {space,dir1,dir2,delta} ({state,ball,player1,player2,bounces} as game) =
  let
    score1 =
    ...
    newBounces =
      if state == Pause then
        ...
        (if (within player1 ball) || (within player2 ball) then bounces + 0.1 else bounces)
    
    newBall =
      if state == Pause then
        ball
      else
        updateBall (delta * newBounces) ball player1 player2
  ...
  in
    { game |
        ...
        player2 = updatePlayerAI delta ball score2 player2,
        bounces = newBounces
    }
...
updatePlayerAI : Time -> Ball -> Int -> Player -> Player
updatePlayerAI dt ball points player = 
  let
    diff = (ball.y + ball.vy * ((abs (player.x - ball.x)) / ball.vx)) - player.y
    vy =
      if ball.vx < 0 || ball.x < 0 then
        (0 - player.y) / (abs player.y) * 200
      else
        if diff < 0 then (if diff < 200 * dt then -200 else 0) else (if diff > 200 * dt then 200 else 0)
    movedPlayer = physicsUpdate dt { player | score = player.score + points, vy = vy }
  in
